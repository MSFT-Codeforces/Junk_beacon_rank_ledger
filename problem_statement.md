...
Note that the physical path is undirected, but the equation direction is fixed as written. Each input line $(u,v,w)$ is an independent constraint; if both $(u,v,w)$ and $(v,u,w')$ appear, both must hold (so $w'=-w$ for consistency). Self-loops $(u,u,w)$ imply $0=w$; multi-edges are allowed if they impose the same constraint.
